...
- [x] 11. Past issues findable on the site — **closed by decision, no code written**
  - Files: none (documented in `email/buttondown/README.md` § Publishing an issue)
  ...
  - **Outcome: no archive.** Matthew's call, 2026-08-17: tag any post that goes out as an issue with
    `newsletter`. `/blog/tags/newsletter` is then the archive, generated by the existing tag system.
    The task's own Restrictions were what stopped this being built as specified — the duplication
    question was real, and answering it deleted the work rather than shaping it.
  - **Verified rather than assumed**: tagged a real published post, ran `velite build` then
    `next build`, and confirmed `.next/server/app/blog/tags/newsletter.html` is produced;
    `getAllTags()` puts it in the sitemap. Reverted; nothing carries the tag yet.
  - **Two traps hit while verifying**, both now in the README: `pnpm build` does not re-run velite
    (`d-a31e2253`, `d-096a531a`), so the first two attempts tested stale content; and `fixture-*`
    posts are excluded from `getVisiblePublishedPosts()`, so the first attempt used a post that
    could never have produced a tag page.
