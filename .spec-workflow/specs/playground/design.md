...
- **`src/app/sitemap.ts`** lists `/playground` (line 14) in a static `routes` array and derives dynamic entries from `src/lib` getters; adding manifest-derived landing routes is an additive change (Req 8.3). **`src/config/site.ts`** `heroCards`/`navItems` already link `/playground` (no change needed).
