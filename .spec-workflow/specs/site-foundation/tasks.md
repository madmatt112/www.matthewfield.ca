...
- [x] 8. Create CSS theme tokens and shadcn/ui scaffolding
...
- [x] 9. Install shadcn/ui components for spike and site shell
...
- [x] 10. Implement CSS isolation spike — container and layer setup
...
- [x] 11. Implement CSS isolation spike — test fixtures
...
- [x] 12. Implement CSS isolation spike — Radix overlay verification
...
- [x] 13. Implement CSS isolation spike — Playwright verification tests
  ...
  - Deferred: the Button background-color / color assertion for point (2) of this task is deferred to task 15 because shadcn's `bg-primary` / `text-primary-foreground` utilities are no-ops until task 15 adds the Tailwind v4 @theme block to globals.css. R11 AC3 is covered in task 13 by a separate tokens-via-var() fixture (Test 5 in playground-isolation.test.ts) that consumes the re-established custom properties directly; task 15's work closes the utility-class path and re-asserts the Button renders with expected values.
...
- [x] 14. Document CSS isolation spike outcome
...
- [x] 15. Complete global styles setup and verify shadcn/ui theming
  ...
  - _Requirements: R9 AC1, R9 AC2, R5 AC5, R9 AC3 (carry-over — runtime-verified in task 16 once @/styles/globals.css becomes the active stylesheet)_
...
- [x] 16. Create root layout with font loading and ThemeProvider
  ...
  - Export default metadata with title template `%s | matthewfield.ca`
  - Switch the root layout stylesheet import from `./globals.css` (Create Next App scaffold at `src/app/globals.css`) to `@/styles/globals.css`, then delete `src/app/globals.css`
  - Carry-over from task 15 review: perform the R9 AC3 Button visual verification in both light and dark themes here, since the `@theme` block only becomes runtime-active once this import flip lands. Render a shadcn Button in a scratch page (or transient story) and confirm `bg-primary` / `text-primary-foreground` resolve, then toggle `document.documentElement.classList.toggle('dark')` in DevTools to verify the dark variant renders
  - Purpose: Establish the root layout with fonts, theming, and base metadata in a single coherent pass
  - _Requirements: R14 AC1, R14 AC2, R14 AC3, R5 AC1, R5 AC4, R10 AC1, R9 AC3 (carry-over from task 15)_
  - _Prompt: Role: Frontend developer building the root layout for a Next.js App Router site | Task: Install next-themes as a dependency. Create src/app/layout.tsx as server component. Import Geist and Geist_Mono from next/font/google with subsets: ["latin"] and variable names --font-sans and --font-mono (adjustFontFallback enabled by default). Wrap children in html element (with font variable classes from Geist/Geist_Mono and suppressHydrationWarning) and body. Create theme-provider.tsx as client component wrapping next-themes ThemeProvider with attribute="class", defaultTheme="system", enableSystem. Wrap body children in ThemeProvider. Import `@/styles/globals.css` (task-15-authored file with @theme inline block) and delete the stale `src/app/globals.css` scaffold. Export metadata with title object (default: "Matthew Field", template: "%s | matthewfield.ca"), description, and openGraph image. Before marking the task complete, perform the task-15 carry-over R9 AC3 visual verification: load any page that uses a shadcn Button, confirm bg-primary / text-primary-foreground resolve via the now-active @theme mapping, then toggle `.dark` on `<html>` via DevTools and re-verify — record the observation in the implementation log | Restrictions: Root layout is a server component, ThemeProvider must be a separate client component, use next/font/google (auto self-hosts at build time, no external font requests), import globals.css from @/styles (not ./globals.css), remove src/app/globals.css after switching the import | Success: Pages render with correct title template, Geist fonts load without layout shift, --font-sans and --font-mono CSS variables available, theme class applied to html without FOUC, shadcn Button renders with task-15 token mappings in both light and dark themes (R9 AC3 carry-over verification recorded in the log)_

- [x] 17. Implement theme toggle component
...
- [x] 18. Create site config and wire metadataBase into root layout
  - File: src/config/site.ts, src/app/layout.tsx
  - Define and export `siteConfig` object with name, description, URL, OG image path, navItems, and heroCards nested inside
  - Carry-over from task 16 review: set `metadataBase: new URL(siteConfig.url)` in the root layout's `metadata` export so the existing `openGraph.images: ["/images/og-default.png"]` relative path resolves against the canonical site URL (not localhost) at build time
  - Purpose: Single source of truth for navigation, hero cards, and site metadata; close the task-16 metadataBase gap
  - _Requirements: R4 AC2, R6 AC4, R10 AC1 (carry-over from task 16 — metadataBase wiring)_
  - _Prompt: Role: Developer creating centralized site configuration | Task: Create src/config/site.ts exporting a single siteConfig object of type SiteConfig containing: name, description, url, ogImage, navItems (array of NavItem with label and href for Professional Profile→/profile, Projects→/projects, Contributions→/contributions, Blog→/blog, Resources→/resources, Playground→/playground), and heroCards (array of HeroCardConfig with title, description, and href for the same sections). Define NavItem, HeroCardConfig, and SiteConfig types with navItems and heroCards nested inside SiteConfig (matching the design doc type definition). Consumers access via siteConfig.navItems and siteConfig.heroCards. After the config is in place, update src/app/layout.tsx to import siteConfig and add `metadataBase: new URL(siteConfig.url)` to the metadata export — this closes the task-16 carry-over where openGraph.images uses a relative path | Restrictions: Types defined in same file (no separate types file needed), descriptions should be concise placeholder text, siteConfig.url must be a fully-qualified https URL so `new URL(...)` parses successfully | Success: Config is importable, types are correct, all 6 sections represented in both navItems and heroCards, single siteConfig export with nested arrays, layout.tsx metadata carries metadataBase derived from siteConfig.url, pnpm build no longer emits the "metadataBase property in metadata export is not set" warning_

- [x] 19. Implement site layout with header, nav, and footer
...
- [x] 20. Configure metadata and SEO conventions
...
- [x] 22. Create landing page with hero cards
...
- [x] 25. Write E2E tests — theme toggle
...
- [x] 26. Write E2E tests — navigation and placeholder pages
...
- [x] 27. Write E2E tests — landing page and hero cards
...
- [x] 28. Write E2E tests — CSP headers
