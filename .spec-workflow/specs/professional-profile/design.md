...
- **Static-first**: Both pages declare `export const dynamic = 'force-static'` (Req 1.9), participating in the project's "all main pages are statically generated at build time" posture.
...
The transitions are pure state changes; the side effects (focus, scroll, fire CustomEvent) run in a `useEffect` whose dependency is a per-attempt counter, NOT `state.kind`. The form keeps an `attemptId: number` value in state that increments on every submit handler call (regardless of outcome), and the effect is keyed on `[attemptId, state.kind]`. This is necessary because React's `useEffect` dependency-array semantics use `Object.is` equality: keying the effect on `state.kind` alone would silently skip same-kind transitions like `validation-error → validation-error` (e.g. a user submits with two empty fields, fixes one, submits again with a different invalid field — the focus-management effect must re-fire to move focus to the new first-invalid input). Pairing `attemptId` (which always changes) with `state.kind` (which captures the outcome) ensures every submit attempt re-runs the effect exactly once.

`<ContactForm />` does NOT use the native `disabled` attribute on the submit button while submitting, because Chrome blurs disabled buttons (focus jumps to `<body>`) and screen readers announce nothing useful — the user is left in focus limbo with no anchor. Instead the button uses `aria-disabled="true"` while in `submitting` state, paired with an early-return guard at the top of `handleSubmit` (`if (state.kind === 'submitting') return;`) to prevent double-submission. `aria-disabled` keeps the button focusable and screen-reader-announced; the in-handler guard preserves the no-double-submit guarantee.
...
- **DOM structure**:
  ...
  - Submit `<Button type="submit" aria-disabled={state.kind === 'submitting'}>` showing "Send" / "Sending…". The submit handler early-returns when `state.kind === 'submitting'` (see form-state machine), preventing double-submission without the focus-loss side effect of native `disabled`.
...
- **Interfaces** (named exports):
  - `export type ContactEmailInput = { name: string; email: string; message: string; source: 'profile' | 'contact' | undefined }`.
...
- **Pipeline implementation order** (matches Req 3.5 a–g):
  ...
  2. Origin/Referer check. The accepted-origin set is computed once per request as a small list rather than a single string:
     ...
     - `Origin` header is present and `new URL(origin).host` matches one of the above (production exact match, `VERCEL_URL` exact match, `*.vercel.app` suffix match, or `localhost` hostname match), OR
     - `Origin` is absent AND `Referer` is present AND `new URL(referer).host` matches one of the above, OR
     - both `Origin` and `Referer` are absent (the both-absent fallback per Req 3.5b).

     Mismatched `Origin` or `Referer` → 403. The fallback firing is NOT logged (Req 3.5b).

     Implementation note: the Vercel-wildcard suffix-match is intentionally permissive (any `*.vercel.app` is allowed) because the only way a user reaches our form on a `*.vercel.app` host is via a deploy *we* shipped — Vercel does not host third-party content under that suffix in a way the user could reach via our DNS. The threat model is "attacker scripts a curl spray," which is already covered by the both-absent fallback in Req 3.5b; tightening the wildcard to a per-deploy hostname list would not improve security and would silently 403 alias hosts.
  ...
  8. `await sendContactEmail({ ...validated, source })` inside `try`/`catch`. On `TimeoutError` → 503 with `Retry-After: 60` header. On other errors → 502 with the user-facing message. Success → 200.
- **Error logging**: only sanitized codes. E.g. `console.warn(\`resend_${err instanceof TimeoutError ? 'timeout' : '4xx_5xx'}\`)`. No `name`, `email`, `message`, `source` ever passed to `console.*`. No `Error` objects with those values in their `.message` thrown out of the handler — caught and replaced with sanitized errors at the boundary (Req 3.10). The `getResendClient()` sanity-guard error (test-key + production base URL) is allowed to bubble as a 500 in production — its message contains no user data, and surfacing the misconfiguration loudly is preferable to silently routing test traffic to real Resend.
...
Three-step fallback: `--deepen=1000` is the preferred shape (works regardless of whether the clone is shallow); `--unshallow` is the older form some Vercel build-image versions accept; `|| true` is the last-resort safety so a network failure during the deepen step does not fail the build. The Velite transform's named-error contract (Req 1.4 transform-failure clause) catches the actual "we still don't have history" case downstream.
...
Two new E2E test files plus a sidecar mock and a wrapper script.
...
- `POST /emails` (Resend's emails endpoint): reads the `X-Test-Id` request header, records `{ testId, body }` to an in-memory map keyed by `testId`. Returns `200 { id: 'mock-<n>' }`.
- `POST /__reset?testId=<id>`: clears the map entry for that `testId` only (or all entries if `testId` is omitted, for one-shot harness reset). Returns `200 { ok: true }`.
...
How the test attaches its id: each Playwright test allocates a UUID in a `beforeEach` hook and sets it on the page via `page.addInitScript((id) => { window.__TEST_ID = id; }, testId)` before navigating. The `<ContactForm />` component reads `window.__TEST_ID` (when present) and includes it as the JSON body field `testId` (NOT a header — client cannot set arbitrary headers without exposing a CORS-affecting surface). The route handler's zod schema `.strip()` mode discards the `testId` field from validated user data, but the handler reads `parsed.testId` directly *before* passing the validated body to `sendContactEmail`, and forwards it as the `X-Test-Id` request header on the outbound fetch. In production, `window.__TEST_ID` is undefined → `testId` field absent → no header forwarded → mock would log under `'__untagged__'` (irrelevant — production never hits the mock).
...
For each of `/profile` and `/contact` (parameterized via `for (const path of ['/profile', '/contact']) { test(...) }`):
