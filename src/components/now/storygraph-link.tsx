...
/**
 * Brand icon as inline SVG with the official path, for the same reason
 * social-links.tsx does it: lucide-react dropped brand marks over trademark,
 * so there is no icon-library source for this one. Path is The StoryGraph's
 * mark from simple-icons (slug `thestorygraph`, icons are CC0); the trademark
 * remains StoryGraph's and this is attribution use — it links to their site.
 *
 * Inline rather than its own module, following LinkedInIcon: only one place
 * renders it. Promote it to a shared module if a second caller appears.
 */
...
/**
 * The reading list here is a hand-mirrored subset — three books per column at
 * most. This is the way through to the full one.
 */
export function StoryGraphLink() {
  return (
    <Button asChild variant="outline" className="mt-6 min-h-11">
      <a href={siteConfig.links.storygraph} target="_blank" rel="noopener external">
        <StoryGraphIcon />
