...
describe("countWordsFromMdast", () => {
  test("drops code blocks AND inline code (75 words)", () => {
    ...
    const paragraphWords = Array.from({ length: 50 }, (_, i) => u("text", `word${i}`));
    ...
    // 25-word heading
    const headingWords = Array.from({ length: 25 }, (_, i) => u("text", `head${i}`));
  ...
  test("whitespace edge", () => {
    const tree = u("root", [u("paragraph", [u("text", "  foo   bar  ")])]) as Root;
  ...
  test("image alt contributes", () => {
    const tree = u("root", [
      u("paragraph", [u("image", { url: "/x.png", alt: "alpha beta gamma" })]),
    ]) as Root;
