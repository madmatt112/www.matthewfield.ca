...
Tasks are ordered so dependencies flow naturally: dependencies → build-time helpers → Velite shared plugin stack → posts collection (split into atomic schema/rejection/reading-time/bodyHtml subtasks) → siteConfig audit → fixtures → query layer → presentation → routes → syndication → verification scaffolding → tests → final integration smoke. Each task references the design section that pins its behavior and the requirements it satisfies.

## Revision history

- **v2 (post-r1 adversarial review)**: Carve out `<!-- ... -->` comment-shaped html nodes in the rejection visitor (Task 4.2) so fixture FIXTURE-NOTE markers do not crash Velite. Split Task 4 into 4.1–4.4 with the `]]>` substitution promoted to a body bullet. Pin literal Shiki target-variable names (`--shiki-light`/`--shiki-dark`) in globals.css — no deferral, no forward dependency. Replace dashboard `<content:encoded>` launch gate with dual assertions: parsed-tree (via `cdataPropName`) AND raw-string CDATA framing. Relocate build-time helpers to `src/lib/build/*.ts` so Vitest's default resolution sees them. Add Task 5 (siteConfig audit/extension for RSS channel-metadata fields). Commit a fixture image asset in Task 6. Pin `BLOG_INCLUDE_DRAFTS` at step level only in Task 20, and pin the e2e server-boot env-propagation contract. Pin `node-html-parser` as the HTML parser for the body-parity test in Task 27 (was three undecided options). Acknowledge that the sentinel check is defense-in-depth (catches the `rm -rf` regression class), not topology proof.
- **v3 (post-r2 adversarial review)**: Add explicit `Depends on:` ordering footers between sibling tasks that have a real ordering edge (Task 6 depends on 4.1, etc.). Pin literal alt text in fixtures (Task 6). Expand Task 4.2 success to include a multi-line FIXTURE-NOTE comment test. Add Task 6.5 — `src/lib/blog-errors.ts` exporting shared error-message constants imported by both `getPublishedPosts()` (Task 7) and `next.config.ts` (Task 16) so the operator-facing runbook lives in one place. Add Vercel-log manual-verification step to Task 16's success criterion (closes the same staleness pattern as design-r3). Move Task 20's `touch sentinel` to immediately after `pnpm build` succeeds (so the sentinel signals "Build 1 reached cleanup" rather than "Build 1 completed every step"); drop the e2e step's "defense-in-depth" framing for the env var and reframe as "forward-compat pin." Add Task 20.5 — `scripts/verify-ci-topology.mjs` (YAML-parses ci.yml and asserts the structural pins r1/r2 surfaced — mechanical defense, parallel to Task 17's ESLint chokepoint). Acknowledge Task 9's pinned variable names ARE structurally verified at Task 28, and pin Task 28 to re-run Tasks 24 + 26 if its spot-check patches globals.css (cite re-run output in the implementation log). Add Task 22.5 — synthetic-input unit test for `countWordsFromMdast` so a walker bug isn't hidden by the identity assertion's same-function-both-sides tautology; pin Task 22's reading-time substrate as `post.body` from `#site/content`. Clarify `_Requirements:` footer semantics ("this task contributes to satisfying these requirements"). Add the Requirements Coverage Matrix at the document foot — orphan requirements become visible.
- **v4 (post-r3 adversarial review, FINAL)**: Promote `checkVercelDraftGuard()` from "optional helper" to required export in Task 6.5; add env-var-name string constants (`VERCEL_ENV_VAR_NAME`, `VERCEL_FLAG_VAR_NAME`, `DRAFT_FLAG_VAR_NAME`) so literal env-var names live once. Disambiguate `code` (block) vs. `inlineCode` (span) in Task 2's walker contract — drop both. Pin Task 22.5's mdast construction substrate to `unist-builder` and pin three separate test cases. Add new Task 6.6 — direct truth-table test for `checkVercelDraftGuard()` in `src/lib/blog-errors.test.ts`. Pin literal `name:` strings for each CI step in Task 20; extend Task 20.5 pin (a) to cover workflow-level env (not just job-level); pin verifier "missing step = fatal exit" semantics; add sibling self-test fixtures for the verifier. Drop Task 28's "only on patch" gate — Tasks 24 + 26 re-run unconditionally as part of the spot-check AND the implementation log captures computed hex values compared against expected github-light/github-dark theme hexes. Pin Playwright report citation format (JSON `results` array fragment). Add new Task 28.5 — `scripts/verify-requirements-coverage.mjs` AND `scripts/verify-task-dependencies.mjs` (mechanical defenses for matrix + dependency graph). Add Vitest `include` pattern verification to Task 22.5. Extend `Depends on:` footers to every remaining task so the graph is complete. Pin Task 21's threshold contract (≥90 across performance/a11y/best-practices/SEO).
...
- [x] 1. Add new runtime/dev dependencies to package.json with exact pins
  ...
  - Purpose: Provide the parser/highlighter/feed/XML+HTML-parser building blocks the design depends on.
  - _Leverage: existing package.json structure_
  - _Requirements: 1.5, 9.0, 9.4, 11.0_
...
- [x] 2. Create build-time helpers under `src/lib/build/`
  ...
  - **Placement decision (per r1 review)**: under `src/lib/build/` (TypeScript, not project-root `.mjs`). The existing `tsconfig.json` `@/*` alias and Vitest's default include/transform already cover `src/`. Importing from velite.config.ts at project root is the standard "TS importing TS sibling" path — Velite's internal esbuild loader handles it.
  - Purpose: Single source of truth for URL absolutization and word counting; shared between Velite transform and Vitest tests with zero resolver gymnastics.
  - _Leverage: existing `src/` tsconfig include path; mdast-util-to-string + unist-util-visit_
  - _Requirements: 6.1, 6.5, 11.8_
  - _Depends on: 1_
...
- [x] 3. Extend velite.config.ts with shared plugin arrays, `markdown` top-level config, `prettyCodeOptions`
  ...
  - Add `markdown: { remarkPlugins: sharedRemarkPlugins, rehypePlugins: sharedRehypePlugins }` to `defineConfig`.
  - Update existing `mdx.rehypePlugins` to use the shared array (replacing `[rehypeSlug]`); add `mdx.remarkPlugins: sharedRemarkPlugins`.
  - Import `rehypeAbsolutizeUrls` and `countWordsFromMdast` from `./src/lib/build/...` (relative paths from project root).
  - Import `siteConfig` from `@/config/site`.
  - Purpose: Establish dual-pipeline emit with identical plugin stacks; foundation for posts collection.
  - _Leverage: existing velite.config.ts structure_
  - _Requirements: 9.0, 9.1, 11.8_
  - _Depends on: 1, 2_
...
- [x] 4. Add the `posts` Velite collection (atomic sub-tasks)
  - **4.1 Schema fields + slug + `.strict()` + kebab-slug regex**
    ...
    - `.strict()` mode rejects unknown frontmatter.
    - Kebab-slug regex enforcement on each tag/category item with the **exact** Req 4.6 error message string.
    - Slug derivation: `data.slug.replace(/^posts\//, "")` (mirrors pages collection pattern).
    - Empty `.transform((d) => d)` placeholder for 4.2/4.3/4.4 to extend.
    - Do NOT register the collection in `defineConfig` yet — 4.4 handles registration.
    - _Requirements: 1.0, 1.6, 4.6_
    - _Depends on: 1, 3_
    - _Design refs: "Velite posts collection" — schema fields + `.strict()` mode pins_
    - _Prompt: Implement the task for spec blog-core, first run spec-workflow-guide to get the workflow guide then implement the task: Role: Velite/Zod schema engineer | Task: Add the posts collection schema (no transform body beyond identity). Mark in-progress; log-implementation when done. | Restrictions: Exact Req 4.6 error message verbatim. Do not register in defineConfig yet — 4.4 registers it after transform chain is complete. | _Leverage: pages collection pattern | _Requirements: 1.0, 1.6, 4.6 | Success: Schema compiles, strict mode rejects unknown keys (verifiable later by 4.2 fixture). Then mark complete._

  - **4.2 Rejection visitor (raw HTML + MDX-only syntax + parse-failure catch + HTML-comment carve-out)**
    - File: velite.config.ts (continue)
    - Inside the posts `.transform()` callback, parse `meta.content` with `remark-parse + remark-gfm + remark-mdx` wrapped in try/catch. On parse throw → throw named `NamedRejectionError(file, 'mdx-parse-failure', err.message)`.
    - Walk the tree throwing named errors on: `mdxJsxFlowElement`, `mdxJsxTextElement`, `mdxFlowExpression`, `mdxTextExpression`, top-level import/export declarations.
    - **`type: 'html'` nodes (per r1 adversarial review, Attack Surface 1)**: throw UNLESS `node.value.trimStart().startsWith('<!--')` AND `node.value.trimEnd().endsWith('-->')`. The carve-out is authorized at the design layer by the design's "Test fixtures" section, which calls for a FIXTURE-NOTE comment header — the comment is the load-bearing exception. All other `type: 'html'` nodes (`<aside>`, `<details>`, `<div>`, etc.) throw `NamedRejectionError(file, 'html-rejected', snippet)`. Document this carve-out in a comment on the visitor with the rationale ("HTML comments are mdast `type: 'html'` per `mdast-util-from-markdown`; the FIXTURE-NOTE soft signal needs to survive the rejection layer").
    - Error message format: names file path, node type (or `mdx-parse-failure`), and leading characters of the snippet.
    - This visitor runs **first** in the transform chain; if it throws, neither reading-time nor bodyHtml ever execute.
    - _Requirements: 3.7, 11.7a_
    - _Depends on: 1, 2, 4.1_
    - _Design refs: "Raw-HTML and MDX-expression rejection" — extended per r1 review Attack Surface 1 to allow comment-shaped html nodes_
    - _Prompt: Implement the task for spec blog-core, first run spec-workflow-guide to get the workflow guide then implement the task: Role: unified/remark mdast engineer | Task: Implement the rejection visitor per the design, with the HTML-comment carve-out from the r1 adversarial review. Mark in-progress; log-implementation when done. | Restrictions: Try/catch wraps the parser invocation — parse failures produce `mdx-parse-failure` named errors, not unnamed stack traces. The HTML-comment carve-out applies ONLY to nodes whose trimmed value is bounded by `<!--` / `-->` — no other html-node values are whitelisted. Inline comment must explain why the carve-out exists. | _Leverage: schema from 4.1 | _Requirements: 3.7, 11.7a | _Depends on: 4.1 | Success: A test post with `<aside>` throws; a test post with `{post.title}` throws; a single-line `<!-- FIXTURE-NOTE: ... -->` does NOT throw; **a multi-line FIXTURE-NOTE spanning a newline (`<!-- FIXTURE-NOTE: line one\nline two -->`) also does NOT throw (per r2 review Attack Surface 1 — the `trimStart().startsWith('<!--')` / `trimEnd().endsWith('-->')` predicate must hold across newlines in the node's `value`)**. Then mark complete._

  - **4.3 Reading-time transform**
    - File: velite.config.ts (continue)
    - After the rejection visitor succeeds, parse `meta.content` with `remark-parse + remark-gfm` (markdown-only stack — NO `remark-mdx`).
    - Pass the mdast to `countWordsFromMdast` (imported from `@/lib/build/word-count` via task 2).
    - Compute `Math.max(1, Math.round(words / 238))`; attach as `readingTime`.
    - _Requirements: 6.0, 6.1, 6.5_
    - _Depends on: 1, 2, 4.2_
    - _Design refs: "Reading-time transform" pin + parser-stack honesty + two-visitor ordering pin_
    - _Prompt: Implement the task for spec blog-core, first run spec-workflow-guide to get the workflow guide then implement the task: Role: unified/remark engineer | Task: Implement the reading-time transform per the design's "Reading-time transform" pin. Mark in-progress; log-implementation when done. | Restrictions: NO remark-mdx in this parser stack. Use the imported `countWordsFromMdast` — do NOT reimplement the walk inline. The Math.max(1, Math.round(...)) clamp lives here. | _Leverage: countWordsFromMdast from task 2 | _Requirements: 6.0, 6.5 | Success: A small known-word-count fixture computes the expected minutes; clamp applied to bodies under one minute. Then mark complete._

  - **4.4 `s.markdown()` bodyHtml + `s.mdx()` body + `]]>` substitution + collection registration**
    - File: velite.config.ts (continue)
    - Add `body: s.mdx()` and `bodyHtml: s.markdown()` to the schema.
    - After `s.markdown()` produces the bodyHtml string, apply the **`]]>` → `]]]]><![CDATA[>` substitution** to bodyHtml inside the transform return value (Req 11.10). This substitution is load-bearing for RSS CDATA safety — the route handler must NOT re-substitute.
    - Register `posts` in `defineConfig({ collections: { pages, profile, posts } })`.
    - _Requirements: 11.10, 11.0_
    - _Depends on: 4.3_
...
- [x] 5. Audit and extend `siteConfig` for RSS channel-metadata fields
  ...
  - Purpose: Close the siteConfig audit gap surfaced by the r1 review's "What's missing" list — Task 22's `validate-feed.mjs` requires `<language>` and will fail CI if siteConfig lacks it.
  - _Leverage: existing src/config/site.ts_
  - _Requirements: 11.0, 11.4_
  - _Depends on: (none — siteConfig pre-exists)_
...
- [x] 6. Create the three fixture posts + commit a placeholder image asset
  ...
  - `fixture-code.mdx`: body contains both ` ```ts ` and ` ```bash ` fenced blocks (multi-line, non-trivial), one inline `code` span, one relative link `/blog/fixture-reading-time` (for the absolutization parity test), one `<h2>` and one `<h3>` for anchor-id parity, one image `![Placeholder fixture image](./fixture-image.svg)`.
  - `fixture-reading-time.mdx`: ≥200 words of prose, both fence flavors, one image `![Placeholder fixture image](./fixture-image.svg)`, one GFM table — substrate for the pure-function identity assertion.
  - **Alt text pinned literally as `Placeholder fixture image`** (per r2 review Attack Surface 1) — both fixtures use the same string so the `includeImageAlt: true` walker's contribution is predictable and downstream-stable across fixture content edits.
  - `fixture-draft-do-not-publish.mdx`: minimal body; title `"Fixture: fixture-draft-do-not-publish"`.
  - Purpose: Stable test substrate referenced by every Vitest and Playwright assertion downstream; closes the r1 review's static-asset audit gap.
  - _Leverage: content/pages/ MDX style_
  - _Requirements: 6.5, 7.13, 7.14, 9.2, 11.9, 11.11_
  - _Depends on: 4.1_
...
- [x] 6.5. Create `src/lib/blog-errors.ts` — single source of truth for env-var names + operator-facing draft-leak guard messages + the predicate helper
  ...
  - **Required named exports (per r3 review Attack Surface 3 — the helper is load-bearing, not "optional")**:
    - **Env-var-name constants** (closes the Vercel-rename staleness path the design-r3 review flagged):
      - `VERCEL_ENV_VAR_NAME = "VERCEL_ENV"`
      - `VERCEL_FLAG_VAR_NAME = "VERCEL"`
      - `DRAFT_FLAG_VAR_NAME = "BLOG_INCLUDE_DRAFTS"`
    - **Message constants** (multi-line template literals — no leading whitespace on each line per the r3 review's prettier/eslint concern):
      - `BLOG_DRAFT_LEAK_GUARD_MSG_PRODUCTION`: verbatim error string for `VERCEL=1 + VERCEL_ENV=production + BLOG_INCLUDE_DRAFTS=1`. Names the three env vars (using the three constants above so the literal string lives once), names the Vercel-side fix, references Req 7.3.
      - `BLOG_DRAFT_LEAK_GUARD_MSG_PREVIEW`: symmetric for the preview-side guard.
    - **Required helper `checkVercelDraftGuard(): { kind: 'production' | 'preview' } | null`**: pure function reading `process.env[VERCEL_FLAG_VAR_NAME]`, `process.env[VERCEL_ENV_VAR_NAME]`, `process.env[DRAFT_FLAG_VAR_NAME]` **at call time**. Returns `{ kind: 'production' }` when Vercel-production-with-drafts; `{ kind: 'preview' }` when Vercel-preview-without-drafts; otherwise `null`. The helper is mandatory: Tasks 7 and 16 both import and call it; reimplementing the predicate inline at either site is a contract violation.
  - Purpose: Close r2 Attack Surface 2 (runbook drift) AND r3 Attack Surface 3 (env-var literal scattering + "optional helper" framing).
  - _Leverage: nothing existing_
  - _Requirements: 7.12_
  - _Design refs: "Operator-error UX" Layers 1 + 2_
  - _Depends on: 1_
...
- [x] 6.6. Implement `src/lib/blog-errors.test.ts` — direct truth-table test for `checkVercelDraftGuard()`
  ...
  - Direct Vitest cases for the helper's behavior:
    ...
    - Assertion of the constants' literal values (`VERCEL_ENV_VAR_NAME === "VERCEL_ENV"`, etc.) so a future rename catches at test time.
    - One assertion that `BLOG_DRAFT_LEAK_GUARD_MSG_PRODUCTION` includes each env-var name as a substring (catches a future edit that drops the name from the runbook).
  - Vitest env-mutation pattern (`beforeEach`/`afterEach` clearing `VERCEL`, `VERCEL_ENV`, `BLOG_INCLUDE_DRAFTS`).
  - Purpose: Closes r3 Attack Surface 3's "happy path tested only implicitly" finding.
  - _Leverage: vitest_
  - _Requirements: 7.12_
  - _Design refs: r3 review Attack Surface 3_
  - _Depends on: 6.5_
...
- [x] 7. Implement `src/lib/blog.ts` query layer + types
  ...
  - Private `byDateDescSlugAsc` comparator used uniformly across index/taxonomy/RSS/neighbors.
  - `formatPostDate` uses `Intl.DateTimeFormat('en-CA', { year, month: 'long', day: 'numeric' })`.
  - `shouldShowUpdatedBadge(post)`: `post.updated != null && new Date(post.updated) > new Date(post.date)`.
  - Purpose: Single chokepoint between Velite content and every consumer.
  - _Leverage: `#site/content` typed alias_
  - _Requirements: 2.0, 4.0, 5.0, 6.0, 7.3, 7.5, 7.6, 7.12, 8.0_
  - _Depends on: 4.4, 6.5_
...
- [x] 8. Apply ThemeProvider symmetric-class config tweak
  ...
  - Purpose: Enable the `:root` + `.dark` + `:not(.light):not(.dark)` cascade in globals.css.
  - _Leverage: existing theme-provider.tsx_
  - _Requirements: 9.2_
  - _Depends on: (none — theme-provider pre-exists)_
...
- [x] 9. Add Shiki dual-theme CSS cascade to `src/styles/globals.css` (pinned literal target-variable names)
  ...
  - Consumer rule: where the plugin emits the color, the cascade's `--shiki-active` resolves to the active theme's value.
  - Purpose: Theme parity for syntax-highlighted code without `!important` battles or forward dependencies.
  - _Leverage: existing tokens.css `:root` + `.dark` pattern; `@custom-variant dark` in globals.css_
...
- [x] 10. Implement blog presentational components
  ...
  - `<TagChip kind value />`: 44×44 min touch target (`min-h-11 min-w-11`), links to `/blog/tags/{value}` or `/blog/categories/{value}`.
  - `<DraftBanner />`: token-driven `destructive` styling, lives inside `<article>` body.
  - All named exports; no barrel files; PascalCase.
  - Purpose: Reusable presentation pieces consumed by the four routes.
  - _Leverage: src/components/ui/card.tsx; tokens.css destructive surface_
  - _Requirements: 2.5, 5.0, 5.3, 5.4, 7.7, 13.4_
  - _Depends on: 1_
...
- [x] 11. Implement `/blog` index route (replacing placeholder)
  ...
  - `generateMetadata()`: title `"Blog"`, description, canonical, RSS `alternate` link, `robots: { index: false, follow: false }` when `VERCEL_ENV === 'preview'`.
  - `export const dynamic = "force-static"`.
  - Empty state: `"No posts yet — check back soon."` inside `max-w-3xl`.
  - Purpose: Reverse-chronological listing; entry point.
  - _Leverage: getPublishedPosts; PostCard_
  - _Requirements: 2.0, 2.9, 7.11a_
  - _Depends on: 4.4, 5, 7, 10_
...
- [x] 12. Implement `/blog/[slug]` post detail route
  ...
  - Body via `<MDXContent />` in `prose prose-lg dark:prose-invert max-w-[75ch]`.
  - Header: title, formatted date, optional updated badge (via `shouldShowUpdatedBadge`), reading time, tag chips, `<DraftBanner />` when draft.
  - Footer: `<PrevNextNav />` via `getPostNeighbors(slug)`.
  - Null post → `notFound()`.
  - _Leverage: src/lib/blog.ts; MDXContent; PrevNextNav, TagChip, DraftBanner_
  - _Requirements: 3.0, 3.6, 3.9, 5.0, 7.7, 7.8, 8.0_
  - _Depends on: 4.4, 5, 7, 10_
...
- [x] 13. Implement taxonomy routes for tags and categories
  ...
  - Each page includes a "back to all posts" link.
  - _Leverage: src/lib/blog.ts; PostCard_
  - _Requirements: 4.0, 4.2, 4.6, 4.8_
  - _Depends on: 4.4, 5, 7, 10_
...
- [x] 14. Implement `/feed.xml` RSS 2.0 route handler + dual-assertion launch gate
  ...
  - `export const dynamic = "force-static"`.
  - **Dual-assertion launch gate (per r1 review Attack Surface 2)**: construct a feed item with `content: '<pre><code>x &amp; y</code></pre>'`, call `rss2()` and capture the **raw string output**. Then assert BOTH:
    - **Raw-string CDATA framing**: `rawOutput.match(/<content:encoded>\s*<!\[CDATA\[<pre><code>/)` matches. This is the only assertion that proves CDATA framing — `fast-xml-parser` unwraps CDATA by default, so parsed-tree text content is identical whether the XML uses CDATA or character escaping. The r1 review's correction adopted.
    - **Parsed-tree content equality (with `cdataPropName: '__cdata'`)**: parse with `new XMLParser({ cdataPropName: '__cdata', ignoreAttributes: false })`, locate `rss.channel.item[0]['content:encoded']`, assert `__cdata` field present AND its value contains the literal `<pre><code>` substring. This proves the HTML survived inside the CDATA wrapper without being XML-escaped.
    - If either assertion fails, the test fails with a diagnostic identifying which side of the dual gate caught it. Both must pass for the launch gate to be green.
  - Purpose: Build-time static RSS feed; dual-gated launch test catches both CDATA framing regressions AND content-escape regressions in `feed@>=4.3`.
  - _Leverage: src/lib/blog.ts; siteConfig (from task 5); feed@4.2.2; fast-xml-parser_
  - _Requirements: 11.0, 11.4, 11.5, 11.9, 11.10_
  - _Depends on: 1, 4.4, 5, 7_
...
- [x] 15. Extend `src/app/sitemap.ts` with posts/tags/categories
  ...
  - All URLs composed via `new URL(path, siteConfig.url)`.
  - _Leverage: existing sitemap.ts structure_
  - _Requirements: 12.0, 12.1, 12.2_
  - _Depends on: 4.4, 5, 7_
...
- [x] 16. Extend `next.config.ts` with preview robots header + early stderr env-var guard (shared error constants)
  ...
  - Extend `headers()`: when `process.env.VERCEL_ENV === 'preview'`, add one route block matching `/(.*)` with `X-Robots-Tag: noindex, nofollow`. Existing CSP block untouched.
  - **Vercel-log surfacing verification (per r2 review Attack Surface 2)**: success criterion includes a manual verification — on a real Vercel preview deploy, deliberately misconfigure one env-var scope (e.g., set `BLOG_INCLUDE_DRAFTS=1` on production), trigger a deploy, **read the actual Vercel build log** and confirm the named error string from `BLOG_DRAFT_LEAK_GUARD_MSG_PRODUCTION` (or PREVIEW) appears legibly there, not just as "Command failed with exit code 1." Document the verification in the implementation log. Repeat the verification quarterly (calendar reminder; not committed to repo) so a future Vercel-side log-format change does not silently degrade the operator UX. This is the same staleness pattern as design-r3 Layer-3.
  - _Leverage: existing headers() implementation; src/lib/blog-errors.ts (Task 6.5)_
  - _Requirements: 7.7a, 7.12_
  - _Depends on: 6.5_
...
- [x] 18. Implement `scripts/verify-production-build.mjs`
  ...
  - Sentinel check: assert `.velite/.build1-sentinel` is absent. **Per r1 review Attack Surface 6**: this check is defense-in-depth — it catches the specific regression class of "someone removes the `rm -rf` step." It is NOT a topology proof (the cleanup is unconditional so the absence is naturally true under the current workflow shape). Documented as such in the script's banner comment so future maintainers understand its scope.
  - Shebang, sync fs, `process.exit(non-zero)` on failure.
  - _Leverage: scripts/run-e2e.mjs convention; Node 22+ fs.glob_
  - _Requirements: 7.14_
  - _Depends on: 4.4, 6_
...
- [x] 19. Implement `scripts/validate-feed.mjs`
  ...
  - Shebang, sync fs, process.exit non-zero on failure, diagnostic naming missing field.
  - _Leverage: scripts/run-e2e.mjs; fast-xml-parser_
  - _Requirements: 11.4, 11.5, 11.12_
  - _Depends on: 14_
...
- [x] 20. Extend `.github/workflows/ci.yml` — dual-build job + step-scoped env vars + literal step `name:` pins
  ...
  - **Literal step `name:` strings (per r3 review Attack Surface 4 — Task 20.5's verifier matches against these exact strings; renames are a breaking change to be applied to BOTH files in the same PR)**:
    - `"Verify CI topology"` — runs scripts/verify-ci-topology.mjs (Task 20.5 wires it as Build-1 first step).
    - `"Build 1 (with drafts)"` — runs pnpm build with step-scoped BLOG_INCLUDE_DRAFTS=1.
    - `"Touch Build 1 sentinel"` — runs immediately after the Build 1 build step succeeds.
    - `"Run e2e (Build 1)"` — runs pnpm test:e2e.
    - `"Validate feed (Build 1)"` — runs scripts/validate-feed.mjs.
    - `"Clean for Build 2"` — runs rm -rf .velite .next with `if: always()`.
    - `"Build 2 (production-mode)"` — runs pnpm build with NO BLOG_INCLUDE_DRAFTS in env.
    - `"Verify production build (Build 2)"` — runs scripts/verify-production-build.mjs.
  - **Build 1**: `"Build 1 (with drafts)"` step running `pnpm build` with `env: BLOG_INCLUDE_DRAFTS: "1"` set on **that step only** (per r1 review Attack Surface 6 — env var MUST NOT be set at job-level OR workflow-level). **Per r2 review Attack Surface 3 — touch the sentinel IMMEDIATELY after `pnpm build` succeeds, before e2e and feed validation run.** The `"Touch Build 1 sentinel"` step signals "Build 1 reached cleanup-eligible state." Followed by `"Run e2e (Build 1)"` step (no `BLOG_INCLUDE_DRAFTS` env). Then `"Validate feed (Build 1)"`.
  - **Clean**: `"Clean for Build 2"` step → `rm -rf .velite .next` with `if: always()`.
  - **Build 2**: `"Build 2 (production-mode)"` step → `pnpm build` (NO `BLOG_INCLUDE_DRAFTS` env). Then `"Verify production build (Build 2)"` → `node scripts/verify-production-build.mjs`. Both steps `if: always()`.
  - `NODE_OPTIONS='--no-warnings=ExperimentalWarning'` applied to both script invocations (as step env).
  - **e2e server-boot pin**: drafts are baked at build-time, so the static output served by `next start` already includes them — no env needed on the e2e step. If a future spec introduces a server-runtime draft-aware code path, add a step-scoped env var THEN AND an integration test mechanically verifying the runtime env contains it.
  - Purpose: Topology contract + step-scoped env contract + sentinel-ordering fix + literal step names so Task 20.5's verifier matches deterministically.
  - _Leverage: existing ci.yml_
  - _Requirements: 7.14_
  - _Design refs: "CI build separation" section + r3 review Attack Surface 4_
  - _Depends on: 18, 19_
...
- [x] 20.5. Implement `scripts/verify-ci-topology.mjs` + self-tests — mechanical defense for CI topology
  ...
  - **Asserts the structural pins (per r1/r2/r3 reviews — pin (a) extended to workflow-level env per r3 Attack Surface 4)**:
    - **(a)** Neither workflow-level (top-level `env:`) NOR job-level `env` blocks contain `BLOG_INCLUDE_DRAFTS`.
    - **(b)** The step `name: "Build 2 (production-mode)"` does NOT have `BLOG_INCLUDE_DRAFTS` in its `env:` block.
    - **(c)** The step `name: "Clean for Build 2"` carries `if: always()`.
    - **(d)** The step `name: "Touch Build 1 sentinel"` runs **before** `"Run e2e (Build 1)"` and `"Validate feed (Build 1)"`.
    - **(e)** The step `name: "Verify production build (Build 2)"` runs AFTER `"Build 2 (production-mode)"` in the same job.
  - **Missing-step is fatal (per r3 review Attack Surface 4)**: if the script CANNOT locate a step matching any expected literal `name:` string, it MUST exit non-zero with a diagnostic naming the missing step name. Do NOT skip the assertion in this case — silent skipping is exactly the regression class that defeats the verifier.
  - **Wired into `"Verify CI topology"` step** — the FIRST step of the CI job (before Build 1's `pnpm build`), so a topology break fails CI immediately.
  - **Self-tests (per r3 review Attack Surface 4)**: `scripts/verify-ci-topology.test.mjs` runs each of the six fixture ci.yml files through the verifier:
    - `good.yml` — well-formed; verifier exits 0.
    - `bad-workflow-env.yml` — promotes BLOG_INCLUDE_DRAFTS to workflow-level env; verifier exits non-zero with the pin (a) diagnostic.
    - `bad-job-env.yml` — promotes BLOG_INCLUDE_DRAFTS to job-level env; verifier exits non-zero with the pin (a) diagnostic.
    - `bad-build2-env.yml` — adds BLOG_INCLUDE_DRAFTS to the Build 2 step env; verifier exits non-zero with pin (b).
    - `bad-cleanup-missing-always.yml` — drops `if: always()` from the cleanup step; verifier exits non-zero with pin (c).
    - `bad-sentinel-order.yml` — moves sentinel-touch to after e2e; verifier exits non-zero with pin (d).
    - `bad-verify-missing.yml` — removes the Build-2 verify step; verifier exits non-zero with pin (e).
  - Purpose: Closes r2 Attack Surface 3 (procedural-as-structural) AND r3 Attack Surface 4 (verifier itself is verified; workflow-level coverage; fatal missing-step).
  - _Leverage: scripts/run-e2e.mjs pattern; `yaml` package (added in Task 1)_
  - _Requirements: 7.14_
  - _Design refs: r2 Reversal #1 + r3 Attack Surface 4_
  - _Depends on: 1, 20_
...
- [x] 21. Extend `lighthouserc.js` with four blog URLs + threshold audit
  ...
  - _Leverage: lighthouserc.js_
  - _Requirements: 13.5_
  - _Design refs: lighthouserc.js mention + r3 Attack Surface 6_
  - _Depends on: 14_
...
- [x] 22. Implement `src/lib/blog.test.ts` Vitest unit suite
  ...
  - Cases per design Testing Strategy → Unit Testing:
    ...
    - `__testing.neighbors` against synthetic 5-post array.
    - Public `getPostNeighbors(slug)` integration assertion against a known fixture.
    - `shouldShowUpdatedBadge` three cases.
    - `getAllTags` / `getAllCategories` dedup + lex sort + empty-safe.
    - Formatters.
    - `wordsToReadingTime(N) === Math.max(1, Math.round(N/238))` for `N ∈ {0,1,237,238,239,1000,10000}`.
    - End-to-end identity assertion using shared `countWordsFromMdast` (imported from `@/lib/build/word-count`) against `fixture-reading-time` body. **Substrate pin (per r2 review Attack Surface 5)**: the test reads the fixture-reading-time post's body via `import { posts } from "#site/content"` then `posts.find(p => p.slug === 'fixture-reading-time')`, then re-parses that post's `body`/`bodyHtml`/source as appropriate to obtain an mdast tree using the same `remark-parse + remark-gfm` stack the Velite transform uses (Task 4.3). The test does NOT read the raw `.mdx` file from disk — that bypasses the Velite frontmatter-strip and produces a different tree than the transform's input. **Pre-requisite**: the test suite requires `pnpm velite build` to have run with fixtures present. Vitest's `globalSetup` runs `velite build` before tests (this task wires that setup file if not already present).
  - `beforeEach`/`afterEach` env-var mutation pattern.
  - _Leverage: vitest; getPublishedPosts; countWordsFromMdast_
  - _Requirements: 6.0, 6.5, 7.5, 7.6, 7.12, 8.0_
  - _Design refs: Testing Strategy → Unit Testing_
...
- [x] 22.5. Implement synthetic-input unit test for `countWordsFromMdast` (walker-self test)
  ...
  - **Three SEPARATE Vitest `test()` cases (per r3 review Attack Surface 5 — the v3 phrasing was ambiguous about whether these were one tree or three)**, each with its own pinned mdast fixture built via `unist-builder`'s `u()` helper (added to devDeps in Task 1 if not already pulled transitively by remark — verify and amend Task 1 if necessary). Using `unist-builder` ensures construction produces faithful mdast (`type` + `children`, optional `value`); hand-built object literals risk missing required fields.
    ...
    - **Case 2 — "whitespace edge"**: tree contains a single paragraph whose text node has `value: "  foo   bar  "`. Assert `countWordsFromMdast(tree) === 2`. Tests the `.split(/\s+/).filter(Boolean)` contract against leading/trailing whitespace and internal runs.
    - **Case 3 — "image alt contributes"**: tree contains a paragraph with one `image` node having `alt: "alpha beta gamma"`. Assert `countWordsFromMdast(tree) === 3`. Tests `includeImageAlt: true`.
  - **Vitest include-pattern verification**: this task's first action is to verify the existing `vitest.config.*` `include` pattern covers `src/lib/build/*.test.ts` (per r3 review Attack Surface 5). If not, update the config in this task before writing the test file. If existing config has no explicit `include` (Vitest's default catches it), document the verification in the implementation log.
  - Purpose: Closes r2 Attack Surface 5 (walker tautology) AND r3 Attack Surface 5 (substrate construction + inlineCode disambiguation + include-pattern).
  - _Leverage: countWordsFromMdast (Task 2); vitest; unist-builder_
  - _Requirements: 6.4_
  - _Design refs: "Shared walk extraction" + r3 Attack Surface 5_
  - _Depends on: 1, 2_
...
- [x] 26. Implement Playwright E2E: no-JS + explicit-override theme
  ...
  - _Leverage: Playwright emulateMedia; THEME_STORAGE_KEY_
  - _Requirements: 9.2, 13.7_
  - _Depends on: 6, 9, 12_
...
- [x] 27. Implement RSS body-parity integration test (pinned `node-html-parser`)
  ...
  - Parse both with `node-html-parser` (pinned in Task 1 — replaces the r1 review's three-undecided-options block).
  - Assert: (a) `<pre><code>` elements present in both with matching Shiki token-class patterns; (b) every `<h2>`/`<h3>` has matching `id`s; (c) every relative `<a href>` / `<img src>` is absolutized in both; (d) inline `<code>` present in both.
  - Carve out from text-node equivalence: code-block text containing legitimate `]]>` is allowed to differ per Req 11.10 — assertion checks element shape and structure, not text-node byte equivalence.
  - _Leverage: #site/content posts; .next/server/app prerender output; node-html-parser_
  - _Requirements: 11.11_
  - _Depends on: 1, 4.4, 12_
...
- [x] 28. End-to-end smoke: full local dual-build verification + mandatory re-runs + hex-value capture
  ...
  - **Run mechanical verifiers (per r3 review Attack Surface 1 + r3 What's Missing)**: `node scripts/verify-requirements-coverage.mjs` AND `node scripts/verify-task-dependencies.mjs` (both from Task 28.5) MUST exit 0.
  - **Spot-check Task 9's literal pinned variable names + ALWAYS-RUN downstream re-runs (per r3 review Attack Surface 6 — drops the v3 "only on patch" gate)**:
    - Open the rendered `/blog/fixture-code` HTML, confirm `--shiki-light` / `--shiki-dark` are emitted by `rehype-pretty-code`.
    - **Capture computed hex values**: use a headless-browser snapshot OR Playwright's `evaluate` to read `getComputedStyle(<first code-block token span>).color` for both `prefers-color-scheme: light` and `dark`. Compare against the expected hex codes for `github-light` and `github-dark` plaintext tokens (Shiki's published theme JSON — pin the expected hexes in the implementation-log entry, e.g., `github-light plaintext = #24292e`, `github-dark plaintext = #e1e4e8` — verify against the actual theme JSON at implementation time).
    - **ALWAYS re-run Task 26 (blog-no-js + blog-shiki-theme) AND Task 24 (blog-axe)** regardless of whether globals.css needed patching. Visual inspection alone is insufficient evidence (per r3 review Attack Surface 6).
  - **Patch-path additional contract**: if the rendered output forces a globals.css patch:
    - Document the patch (diff) and the diagnosing observation in the implementation log.
    - The Task 24 + 26 re-runs above are now the regression check on the patched cascade.
  - **Citation format (per r3 review Attack Surface 6)**: implementation log entry MUST cite the Playwright re-runs by their **JSON `results` array fragment** (`results[i].title` + `results[i].status` + `results[i].duration` per test), not console-line "tests passed." Use `pnpm test:e2e --reporter=json` (or save the existing reporter's JSON output) and paste the relevant array slice into the implementation log.
  - Purpose: Final integration gate. Always-run downstream re-runs + hex-value capture + JSON-fragment citation are the mechanical evidence that replaces visual-inspection trust.
  - _Leverage: tasks 1–28.5_
  - _Requirements: All (integration verification)_
  - _Design refs: "CI build separation" + Testing Strategy + r3 Attack Surface 6_
...
- [x] 28.5. Implement mechanical verifiers for the Requirements Coverage Matrix + the Depends-on dependency graph
  ...
  - **`scripts/verify-requirements-coverage.mjs`**:
    ...
    - Extracts coverage matrix entries from `.spec-workflow/specs/blog-core/tasks.md` (between the `## Requirements Coverage Matrix` heading and the next `## ` heading or EOF).
    - Exits non-zero with a named diagnostic for: (a) any requirement ID in requirements.md NOT cited in the matrix; (b) any ID cited in the matrix that no longer appears in requirements.md; (c) any matrix entry pointing at a task number that does not exist in tasks.md.
    - On exit 0, writes a one-line summary to stdout (N requirements matched, M tasks referenced).
  - **`scripts/verify-task-dependencies.mjs`**:
    - Parses tasks.md for `Depends on:` footers (one regex; pin in the script).
    - Builds a dependency graph; validates each cited task number resolves to an actual task in the file; topologically sorts; exits non-zero on cycles, dangling references, or self-references with a named diagnostic.
    - On exit 0, writes a one-line summary (N tasks, M edges, topological order verified).
  - **Wired into Task 28's success criterion**: both scripts MUST exit 0 before Task 28 can be marked complete.
  - Purpose: Closes r3 review Attack Surface 1 (coverage matrix staleness) and Attack Surface 2 (`Depends on:` inconsistency) with mechanical defenses parallel to Task 20.5's CI-topology verifier and Task 17's ESLint chokepoint.
  - _Leverage: scripts/run-e2e.mjs pattern; Node built-in fs + regex_
  - _Requirements: 7.14_
  - _Design refs: r3 review Top 3 Reversals #1 + #2_
  - _Depends on: 28 (the matrix and depends-on footers must exist before the verifiers can validate them — wait, 28 depends on this; so this depends on the matrix/footers existing in tasks.md itself, which they do as of v4 — no inter-task code dependency, just doc state). Effectively: this task can be implemented in parallel with any other; ordering with Task 28 is that 28 wires the invocations._
  - _Prompt: Implement the task for spec blog-core, first run spec-workflow-guide to get the workflow guide then implement the task: Role: Tooling engineer with markdown/regex experience | Task: Implement the two verifier scripts per v4. Mark in-progress; log-implementation when done. | Restrictions: NO external dependencies — Node built-ins + regex only. Each verifier's failure diagnostic must name the specific orphan ID or dangling task number. Both scripts produce a clear stdout summary on success. | _Leverage: scripts dir; markdown source files | _Requirements: 7.14 | Success: Both scripts exit 0 against the current tasks.md / requirements.md; planted orphan ID fails verify-requirements-coverage; planted cycle fails verify-task-dependencies. Then mark complete._

---

## Requirements Coverage Matrix

Inverse mapping from `requirements.md` IDs → covering task(s). Closes r2 review Attack Surface 6 by making orphan requirements visible at review time. Format: `Req X.Y — short tag — covered by: Task N(s)`.
