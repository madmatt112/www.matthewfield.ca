...
- [x] 0. (Precondition — design-phase spike, included for traceability only) Pre-implementation pipeline spike — `Implementation Logs/task-0-spike.md`
...
- [x] 1. Add new dependencies to package.json with exact pins
  - File: package.json
...
- [x] 2. Add `.github/dependabot.yml` (or extend existing) to route `vercel` + `pagefind` separately
...
- [x] 3. Add `public/pagefind/` and `./out` to `.gitignore`
  - File: .gitignore
...
- [x] 4. Create `src/lib/build/derive-post-slug.mjs` — shared slug helper + `KNOWN_FIXTURE_SLUGS` roster
...
- [x] 4.1. Create `src/lib/build/derive-post-slug.test.mjs` — five-case unit suite
...
- [x] 5. Define `PROD_LIKE_PORT` literal in three places (deliberate non-centralization)
...
- [x] 6.1. Velite `posts` schema additions only — `hiddenFromLists`, `excludeFromSearch`, `categories.max(3)` cap
...
- [x] 6.2. Velite `posts.transform` — h4 rejection + `BLOG_ALLOW_H4` escape hatch
...
- [x] 6.3. Velite `posts.transform` — fixture-slug audit + `derivePostSlug` call-site swap
  - File: velite.config.ts (transform body)
...
- [x] 6.4. Series-order collision — Velite-API investigation (LOG-ONLY task; no code change)
...
- [x] 6.4.1. Series-order collision — implementation (Velite hook OR post-build script)
...
- [x] 6.5. `safeBodyHtml` extension — `data-copy-source` RSS strip
...
- [x] 7. Create the seven blog-enhanced fixture posts + extend `fixture-code.mdx`
...
- [x] 8. Extend `src/lib/blog.ts` — visibility filter, series, related, TOC, JSDoc warning
...
- [x] 8.1. Carve taxonomy helpers into `src/lib/blog-taxonomy.ts`
...
- [x] 9. Implement `scripts/run-pagefind-crawl.mjs` — crawl orchestrator with master timeout
...
- [x] 10. Implement `scripts/verify-pagefind-artifact.mjs` — recursive-checksum artifact preservation check
...
- [x] 11. Implement `scripts/verify-pagefind-no-drafts.mjs` — draft-leak + non-empty-index smoke check
...
- [x] 12. Implement `scripts/verify-deploy.mjs` — Req 0.3 v4 verification gate
...
- [x] 13. Implement `scripts/check-vercel-auto-deploy.mjs` — migration grace-period gate
...
- [x] 14. Implement `scripts/warn-no-pagefind.mjs` — kill-switch warning + persistent issue
...
- [x] 15. Implement `src/lib/build/rehype-copy-button.ts` — copy-button injection plugin
...
- [x] 15.1. Implement `src/lib/build/rehype-copy-button.test.ts` — plugin unit suite (incl. non-ASCII + RSS-strip)
...
- [x] 16. Wire `rehypeCopyButton` into `sharedRehypePlugins` (BEFORE `rehype-pretty-code`)
  - File: velite.config.ts
...
- [x] 17. Implement `src/components/blog/clipboard.ts` — UTF-8-safe decode + status announce helper
...
- [x] 17.1. Implement `src/components/blog/clipboard.test.ts` — UTF-8 round-trip suite
...
- [x] 17.5. Carve `src/styles/globals.css` into per-component CSS slice files via `@import` + visual-neutrality assertion (v3; v4 — split out preview-route infrastructure to Task 17.6)
...
- [x] 17.6. Preview-route infrastructure + per-component registry FILES (v4 — carved out of v3's monolithic 17.5; per-component registry closes the registry.ts merge-conflict factory r3 attack 1 surfaced)
...
- [x] 18.1. `<SeriesBadge />` (server component) + its CSS slice
...
- [x] 18.2. `<SeriesNavigator />` (server component) + its CSS slice
...
- [x] 18.3. `<RelatedPosts />` (server component) + its CSS slice
...
- [x] 18.4. `<ShareBar />` + `<CopyURLButton />` (server + client island) + its CSS slice
...
- [x] 18.5. `<ReadingProgress />` (client component) + reading-progress CSS tokens
...
- [x] 18.6. `<TableOfContents />` (server component) + indentation CSS slice
...
- [x] 18.7. `<CopyButton />` (client hydrator) + code-block-wrapper CSS slice
...
- [x] 18.8. Footnote section styling (CSS-only slice)
...
- [x] 18.9. Shared theme-parity tokens + `.pagefind-ui` overrides (CSS-only slice)
...
- [x] 19. Implement `<SiteSearch />` client component + `<noscript>` hide CSS
...
- [x] 20. Integrate new components into `src/app/(site)/blog/[slug]/page.tsx`
...
- [x] 21. Flip list-context callers to `getVisiblePublishedPosts()`
...
- [x] 22. Extend `next.config.ts` with defensive Velite import + `X-Robots-Tag` header for hidden posts
  - File: next.config.ts
...
- [x] 23.1. ci.yml — Pagefind crawl + verify-index + manifest-upload steps
...
- [x] 23.2. ci.yml — Vercel build + verify-artifact + deploy steps + `Verify getPublishedPosts callers` insertion
...
- [x] 23.3. ci.yml — `Warn deploying without Pagefind` step + PR-comment escalation + flip Task 25's transitional flag to required
...
- [x] 24. Create `.github/workflows/verify-vercel-token.yml` — weekly token auth check
...
- [x] 25. Extend `scripts/verify-ci-topology.mjs` + `scripts/verify-task-dependencies.mjs` for v2/v3 sub-IDs and transitional-flag gating
...
- [x] 26. Implement `scripts/verify-getPublishedPosts-callers.mjs` + companion Vitest test
...
- [x] 27. Extend `src/lib/blog.test.ts` for visibility filter, series, related, TOC
...
- [x] 28. Implement `src/lib/visibility-truth-table.test.ts` — four-row matrix
...
- [x] 29. Implement `velite-safe-body-html.test.ts` — `data-copy-source` RSS strip
...
- [x] 30. Implement Vitest integration tests for the three new verifier scripts
...
- [x] 31. Implement Playwright `blog-search.test.ts` — Build 2 keyboard + mobile suite
...
- [x] 32. Implement Playwright `blog-pagefind-failure-matrix.test.ts` — three failure modes
...
- [x] 33. Implement Playwright `blog-toc.test.ts` — two-axis parity (Build 1 + Build 2)
...
- [x] 34. Implement Playwright `blog-share.test.ts`, `blog-related.test.ts`, `blog-series.test.ts`
...
- [x] 35. Extend Playwright `blog-no-js.test.ts` + `blog-axe.test.ts` for new surfaces
...
- [x] 36. Extend `lighthouserc.js` — `/blog/fixture-toc` URL + `total-byte-weight` assertMatrix
...
- [x] 37. Documentation: extend `design.md` operator notes + update `README.md`
...
- [x] 38. End-to-end smoke — local dual-build + Pagefind crawl + all verifiers + Playwright re-runs
...
- [x] 39. Mount `<SeriesBadge />` on taxonomy pages (Req 2.7 coverage — v2)
...
- [x] 40. Playwright bounding-box assertion for share-bar 44×44 touch targets (Req 6.6 coverage — v2)
...
- [x] 41. Playwright `prefers-reduced-motion: reduce` emulation for reading-progress bar (Req 5.5 coverage — v2)
